...
@dataclass
class ServerConfig:
    host: str = field(default_factory=lambda: os.getenv("MINI_REDIS_HOST", "0.0.0.0"))
    port: int = field(default_factory=lambda: int(os.getenv("MINI_REDIS_PORT", "6379")))
    ...
    snapshot_path: Path = field(
        default_factory=lambda: Path(os.getenv("MINI_REDIS_SNAPSHOT_PATH", "data/snapshot.json"))
    )
    snapshot_interval_seconds: float = field(
        default_factory=lambda: float(os.getenv("MINI_REDIS_SNAPSHOT_INTERVAL_SECONDS", "30.0"))
    )
